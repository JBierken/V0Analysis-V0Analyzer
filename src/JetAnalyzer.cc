...
void JetAnalyzer::beginJob(TTree* outputTree)
{
    ...
    if( myAnalyzer->storeJecSourcesAll ){
    ...
    if( myAnalyzer->storeJecSourcesGrouped ){
...
bool JetAnalyzer::analyze(const edm::Event& iEvent)
{
    ...
    /*
    edm::Handle<std::vector<pat::Jet>> jetsSmeared     = getHandle(iEvent, myAnalyzer->jetSmearedToken);
    edm::Handle<std::vector<pat::Jet>> jetsSmearedUp   = getHandle(iEvent, myAnalyzer->jetSmearedUpToken);
    edm::Handle<std::vector<pat::Jet>> jetsSmearedDown = getHandle(iEvent, myAnalyzer->jetSmearedDownToken);
    */
