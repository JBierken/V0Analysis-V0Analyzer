...
GenAnalyzer::GenAnalyzer(const edm::ParameterSet& iConfig, V0Analyzer* myAnalyzer):
...
void GenAnalyzer::beginJob(TTree* outputTree, edm::Service<TFileService>& fs){
    ...
    if(myAnalyzer->storeGenParticles){
...
void GenAnalyzer::analyze(const edm::Event& iEvent){
    ...
    for(const reco::GenParticle& p : *genParticles){
        ...
        if((p.status() == 1 and (absId == 11 or absId == 13)) 
                || (p.status() == 2 and p.isLastCopy() and absId == 15)){
            if(_gen_nL != gen_nL_max){
                ...
                //if(myAnalyzer->storeHNLgenInfo) _gen_lProvenanceHNL[_gen_nL] = GenTools::provenanceHeavyNeutrino(p, *genParticles);
